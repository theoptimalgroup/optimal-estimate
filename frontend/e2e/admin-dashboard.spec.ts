...
test.describe("Admin dashboard", () => {
  ...
  test("View all links navigate to filtered eWorks sync quotes tab", async ({ page }) => {
    await mockAuthMe(page, "admin");
    await mockDashboardApi(page);
    ...
    await expect(page).toHaveURL(/\/admin\/eworks-sync\?tab=quotes&status=1/);
    await expect(page.getByTestId("eworks-sync-quotes-table")).toBeVisible();
    await expect(page.getByRole("cell", { name: "Q-NEW" })).toBeVisible();

    await page.goto("/admin/dashboard");
    await page.getByTestId("view-all-awaiting_supplier").click();
    await page.waitForURL(/\/admin\/eworks-sync/);
    {
  ...
  test("search input visible and filters bucket cards", async ({ page }) => {
    ...
    await page.getByTestId("dashboard-search-input").fill("Q-AWAIT");
    await expect(page.getByTestId("dashboard-quote-card-2")).toBeVisible({ timeout: 10000 });
    await expect(page.getByTestId("dashboard-quote-card-1")).toHaveCount(0);
    await expect(page.getByTestId("dashboard-quote-card-3")).toHaveCount(0);
    await expect(page.getByTestId("category-awaiting-supplier-count")).toContainText("Showing 1 of 1");
    await expect(page.getByTestId("category-new-quotes-empty")).toContainText(
  ...
  test("clear search restores all bucket cards", async ({ page }) => {
    ...
    await page.getByTestId("dashboard-search-input").fill("Q-READY");
    await expect(page.getByTestId("dashboard-quote-card-1")).toHaveCount(0, { timeout: 10000 });
  ...
  test("New Estimate link present for e2e flow", async ({ page }) => {
    await mockAuthMe(page, "admin");
    await mockDashboardApi(page);
    await page.goto("/admin/dashboard");
  ...
  test("admin tools section shows secondary stats", async ({ page }) => {
    await mockAuthMe(page, "admin");
    await mockDashboardApi(page);
    await page.goto("/admin/dashboard");
    const tools = page.getByTestId("admin-tools-section");
    await expect(tools).toBeVisible();
    await expect(tools.getByText("Users", { exact: true })).toBeVisible();
    await expect(tools.getByText("Audit logs", { exact: true })).toBeVisible();
  });

  test("manager blocked from admin dashboard page", async ({ page }) => {
